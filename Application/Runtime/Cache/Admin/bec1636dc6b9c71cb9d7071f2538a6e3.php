...
<!--左侧导航栏-->
    <div class="main-content">
        ...
        <div id="commodity_details">
            <h1><img src="/axd/Public/back_img/ico.png"><?php if(empty($_GET['id'])): ?>添加<?php else: ?>修改<?php endif; ?>文章
                <a href="javascript:history.go(-1);"><span><img src="/axd/Public/back_img/return.png"></span></a></h1>
            <div id="commodity_list">
                <!--通用信息_开始-->
                <form action="/axd/Admin/Subject/addEditSubject" method="post" id="goods_form" enctype="multipart/form-data" >

                    <div id="commodity_main" class="clear">
                        <table>
                            <input type="hidden" value="<?php echo ($subject['subject_id']); ?>" name="id">
                            <tr>
                                <th>文章分类：</th>
                                <td>
                                    <select name="s_cate_id">
                                        <?php if(is_array($subject_cate)): foreach($subject_cate as $key=>$c): ?><option value="<?php echo ($c['s_cate_id']); ?>"
                                        <?php if($c['s_cate_id'] == $subject['s_cate_id']): ?>selected="selected"<?php endif; ?>
                                        ><?php echo ($c["name"]); ?></option><?php endforeach; endif; ?>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <th>图片：</th>
                                <td>
                                    <?php if(empty($subject['image'])): ?><img src="" id="to_image">
                                        <?php else: ?>
                                        <img src="/axd<?php echo ($subject["image"]); ?>" id="to_image"><?php endif; ?>
                            ...
                            <tr>
                                <th>详情图片：</th>
                                <td>
                                    <?php if(empty($subject['image_content'])): ?><img src="" id="to_image_content">
                                        <?php else: ?>
                                        <img src="/axd<?php echo ($subject["image_content"]); ?>" id="to_image_content"><?php endif; ?>
                            ...
                            <tr>
                                <th>文章名称：</th>
                                <td><input type="text" value="<?php echo ($subject["name"]); ?>" name="name"></td>
                            </tr>
                            <tr>
                                <th>文章介绍：</th>
                                <td><input type="text" value="<?php echo ($subject["text_content"]); ?>" name="text_content"></td>
                            </tr>
                            <tr>
                                <th>所属性别：</th>
                                <td>
                                    <select name="sex">
                                        <option value="1"
                                        <?php if(1 == $subject['sex']): ?>selected="selected"<?php endif; ?>
                                        >男</option>
                                        <option value="2"
                                        <?php if(2 == $subject['sex']): ?>selected="selected"<?php endif; ?>
                                        >女</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <th>发布者昵称：</th>
                                <td><input type="text" value="<?php echo ($subject["user_nickname"]); ?>" name="user_nickname"></td>
                            </tr>
                            <tr>
                                <th>发布者头像：</th>
                                <td class="user_image">
                                    <?php if(empty($subject['user_image'])): ?><img src="" id="to_user_image">
                                        <?php else: ?>
                                        <img src="/axd<?php echo ($subject["user_image"]); ?>" id="to_user_image"><?php endif; ?>
                            ...
                            <tr>
                                <th>发布者简介：</th>
                                <td><input type="text" value="<?php echo ($subject["user_introduce"]); ?>" name="user_introduce"></td>
                            </tr>
                            <tr>
                                <th>评论数：</th>
                                <td><input type="text" value="<?php echo ($subject["comment_count"]); ?>" name="comment_count"></td>
                            </tr>
                            <tr>
                                <th>收藏数：</th>
                                <td><input type="text" value="<?php echo ($subject["collect_count"]); ?>" name="collect_count"></td>
                            </tr>
                            <tr>
                                <th>分享数：</th>
                                <td><input type="text" value="<?php echo ($subject["share_count"]); ?>" name="share_count"></td>
                            </tr>
                        </table>
                        <!-- 商品列表-start -->
                        <table class="table table-bordered" id="table_s_goods">
                            ...
                            <tbody>
                            <?php if(is_array($goods_list)): foreach($goods_list as $key=>$g): ?><tr class="data_gl_<?php echo ($g["goods_id"]); ?>">
                                    <td>
                                        <?php echo ($g["goods_id"]); ?>
                                        <input type="hidden" value="<?php echo ($g["goods_id"]); ?>" name="goods_lis[]">
                                    </td>
                                    <td><?php echo ($g["name"]); ?></td>
                                    <td><img src="/axd<?php echo ($g["image"]); ?>"></td>
                                    <td><input type="text" name="text_content[]" value="<?php echo ($g["text_content"]); ?>"></td>
                                    <td>
                                        <a href="javaScript:void(0);" onclick="delSGoods('<?php echo ($g["goods_id"]); ?>');">删除</a>
                                    </td>
                                </tr><?php endforeach; endif; ?>
                            <?php if(is_array($_SESSION['goods_temp'])): foreach($_SESSION['goods_temp'] as $key=>$g): ?><tr class="data_gl_<?php echo ($g["goods_id"]); ?>">
                                    <td>
                                        <?php echo ($g["goods_id"]); ?>
                                        <input type="hidden" value="<?php echo ($g["goods_id"]); ?>" name="goods_lis[]">
                                    </td>
...
<script>
    ...
    $('#li1_btn').click(function(){
        $('#goods_form').ajaxSubmit({
            url:"/axd/Admin/Subject/addEditSubject",
            dataType: "json",
            beforeSend: function() {
            ...
            success : function(data){
                alert(data.msg);
                if(0==data.errorCode){
                    location.href = "<?php echo U('Admin/Subject/subjectList');?>";
//                    location.href = document.referrer;
    ...
    function toAddGoods(){
        $('#goods_form').ajaxSubmit({
            ...
            success : function(data){
                if(0==data.errorCode){
                    window.location.href = "<?php echo U('Admin/Subject/addSubjectGoodsPage');?>";
                }else{
                    alert('数据错误');
                }
            }
