    ...
    <div class="main-content">
        <div class="breadcrumbs" id="breadcrumbs">
            <ul class="breadcrumb">
                ...
                <li class="active">搭配评论</li>
            </ul><!-- .breadcrumb -->
        </div>
        <div id="myTabContent" class="tab-content">
            <div class="tab-pane fade in active" id="home">
                <div class="col-sm-4 col-xs-6">
                    <form class="bs-example bs-example-form" action="/axd/Admin/Collocation/collocationComment" id="sf">
                        <div class="input-group" style="margin-bottom: 3%;">
                            <input type="text" class="form-control" placeholder="评论内容搜索" value="<?php echo ($keyword); ?>" name="keyword">
                            <a onClick="document.getElementById('sf').submit();" class="input-group-addon"><i class="icon-search"></i></a>
                        </div>
                    </form>
                </div>
                <a href="<?php echo U('Collocation/exportCollocationComment');;?>" class='btn btn-sm btn-primary pull-left'>导出Excel</a>
            </div>
            ...
            <table class="table table-bordered">
                ...
                <?php if(is_array($list)): foreach($list as $k=>$l): ?><tr>
                        <td><?php echo ($k+1); ?></td>
                        <td>
                            <?php if(empty($l['u_image'])): ?><img src="/axd/Public/img/default_headicon.png">
                                <?php else: ?>
                                <img src="/axd<?php echo ($l["u_image"]); ?>"><?php endif; ?>
                        </td>
                        ...
                        <td class="operation">
                            <a href="javaScript:void(0);" onclick="delCollocationComment('<?php echo ($l["c_comment_id"]); ?>');">删除</a>
                        </td>
...
<script>
    ...
    $(".plgl .dppl").addClass("active");
</script>
<script>
    //删除搭配评论
    function delCollocationComment(id){
        if(confirm("是否删除该数据")){
            $.post("<?php echo U('Admin/Collocation/delCollocationComment');?>",{'id':id},function(data){
                alert(data.msg);
                if('0'==data.errorCode){
