...
<!--头部文件-->
    ...
    <script type="text/javascript">
        ...
        var CONTROLLER = "/axd/Admin/Integral";
        var ACTION = "/axd/Admin/Integral/iOrderList";
    </script>
</head>
    ...
    <div class="main-content">
        <div class="breadcrumbs" id="breadcrumbs">
            <ul class="breadcrumb">
                ...
                <li class="active">兑换订单列表</li>
            </ul><!-- .breadcrumb -->
        </div>
        <div id="myTabContent" class="tab-content">
            <div class="tab-pane fade in active" id="home">
                <div class="col-sm-4 col-xs-6">
                    <form class="bs-example bs-example-form" action="/axd/Admin/Integral/iOrderList" id="sf">
                        <div class="input-group" style="margin-bottom: 3%;">
                            <input type="text" class="form-control" placeholder="商品名称/联系人搜索" value="<?php echo ($keyword); ?>" name="keyword">
            ...
            <table class="table table-bordered">
                ...
                <?php if(is_array($list)): foreach($list as $k=>$l): ?><tr>
                        <td><?php echo ($k+1); ?></td>
                        <td><?php echo ($l["good_name"]); ?></td>
                        <td><?php echo ($l["link_name"]); ?></td>
                        <td><?php echo ($l["link_phone"]); ?></td>
                        <td><?php echo ($l["address"]); ?></td>
                        <td><?php echo ($l["admin_note"]); ?></td>
                        <td>
                            <?php if($l['type'] == 0): ?><p style="color:red;">未处理</p>
                                <?php else: ?>
                                <p style="color:green;">已处理</p><?php endif; ?>
                        </td>
                        <td class="operation">
                            <!--<a href="javaScript:void(0);" onclick="dealTbOrder('<?php echo ($l["id"]); ?>');">处理</a>-->
                            <?php if($l['type'] == 0): ?><a href="<?php echo U('Integral/dealIOrder',array('io_id'=>$l['i_order_id']));?>">处理</a><?php endif; ?>
                        </td>
...
<script>
    $(".jfgl").addClass("active open");
    $(".jfgl .dhddlb").addClass("active");
</script>
<script>
    //
    function dealTbOrder(id){
        if(confirm("是否删除该数据")){
            $.post("<?php echo U('Admin/Law/delLaw');?>",{'id':id},function(data){
                alert(data.msg);
                if('0'==data.errorCode){
