...
<!--头部文件-->
    ...
    <script src="/axd/Public/back_js/subject_goods.js"></script>
    <style>
        #commodity_main input[type=file]{margin-bottom: 10px;}
        #commodity_main img{width: 190px;height: 100px;}
        .cate_g{float: left;margin: 10px;}
        .cate_g p{margin-bottom: 5px;}
        .table-bordered td img{width: 40px;height: 40px;}
    </style>
</head>
    ...
    <div class="main-content">
        <div class="breadcrumbs" id="breadcrumbs">
            <ul class="breadcrumb">
                ...
                <li class="active">编辑文章商品</li>
            </ul><!-- .breadcrumb -->
        </div>

        <div id="commodity_details">
            <h1><img src="/axd/Public/back_img/ico.png">编辑文章商品
            ...
            <div id="commodity_list">
                ...
                <div id="commodity_main" class="clear">
                    <table>
                        <tr>
                            <th>图片：</th>
                            <td>
                                <img src="/axd<?php echo ($subject["image"]); ?>">
                                <input type="text" value="<?php echo ($subject["name"]); ?>" name="name" disabled="disabled">
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                <div>
                    <form action="/axd/Admin/Subject/editSubjectGoods" method="post" id="cate_g_form">
                        ...
                        <div class="cate_g">
                            <p>三级分类</p>
                            <select id="edit_third">
                            </select>
                        </div>
                        <div class="cate_g cate_g_name">
                            <p>商品名称:</p>
                            <select id="goods_list_v" name="gid">
                            </select>
                        </div>
                            <input id="li1_btn" type="button" value="添加" onclick='addSubjectGoods();'/>
                    </form>
                </div>
                <table class="table table-bordered" id="table_s_goods">
                    ...
                    <tbody>
                    <?php if(is_array($goods_list)): foreach($goods_list as $key=>$g): ?><tr data-sg_id="<?php echo ($g["sg_id"]); ?>">
                            <td><?php echo ($g["goods_id"]); ?></td>
                            <td><?php echo ($g["name"]); ?></td>
                            <td><img src="/axd<?php echo ($g["image"]); ?>"></td>
                            <td>
                                <a href="javaScript:void(0);" class="sg_del">删除</a>
                            </td>
...
<script>
    ...
    //添加专题商品
    function addSubjectGoods(){
        $.ajax({
            type: 'POST',
            url: "/axd/Admin/Subject/editSubjectGoods",
            data: $('#cate_g_form').serialize(),
            dataType: "json",
            success: function(data) {
                alert(data.msg);
                if(0==data.errorCode){
                    var goods = data.data;
                    var str = '<tr data-sg_id="'+goods.sg_id+'"><td>'+goods.goods_id+'</td><td>'+goods.name+'</td><td>';
                    str += '<img src="/axd'+goods.image+'"></td><td><a href="###">删除</a></td></tr>';
                    $('#table_s_goods').append(str);
                }
            }
        });
    }
    $(function(){
        $('.sg_del').click(function(){
            var sg_id = $(this).parent().parent().attr('data-sg_id');
            if(confirm("确定要删除该数据吗？")){
                $.post("<?php echo U('Admin/Subject/delSubjectGoods');?>",{'sg_id':sg_id},function(data){
                    alert(data.msg);
                    if(data.errorCode == 0){
