        ...
        <div id="myTabContent" class="tab-content">
            <div class="tab-pane fade in active" id="search">
                <!--<div class="col-sm-4 col-xs-6">
                    <form class="bs-example bs-example-form" action="/axd/Admin/Goods/goodsCate" id="sf">
                ...
                <a href="<?php echo U('Admin/Merchant/addEditGCate');;?>" class='btn btn-sm btn-primary pull-left'>添加</a>
            </div>
            ...
            <table class="table table-bordered" id="list-table">
                ...
                <tbody>
                <?php if(is_array($list)): foreach($list as $k=>$c): ?><tr role="row" align="center" class="<?php echo ($c["level"]); ?>" id="<?php echo ($c["level"]); ?>_<?php echo ($c["g_cate_id"]); ?>">
                    <td><?php echo ($c["g_cate_id"]); ?></td>
                    <td style="padding-left:<?php echo ($c[level] * 5); ?>em;text-align:left;">
                        <?php if($c["level"] != 3): ?><span class="glyphicon glyphicon-minus btn-warning" style="padding:2px; font-size:12px;"  id="icon_<?php echo ($c["level"]); ?>_<?php echo ($vo["id"]); ?>" aria-hidden="false" onclick="rowClicked(this)" ></span>&nbsp;<?php endif; ?>
                        <span><?php echo ($c["name"]); ?></span>
                    </td>
                    <td>
                        <?php if(!empty($c['image'])): ?><img src="/axd<?php echo ($c["image"]); ?>"><?php endif; ?>
                    </td>
                    <td class="operation">
                        <?php if($c['sys_value'] != 1): ?><a href="<?php echo U('Admin/Merchant/addEditGCate',array('id'=>$c['g_cate_id']));?>">编辑</a>
                            <a href="javaScript:void(0);" onclick="delGCate('<?php echo ($c["g_cate_id"]); ?>')">删除</a><?php endif; ?>
                    </td>
...
<script>
    ...
    function delGCate(id){
        if(confirm("是否删除该数据")){
            $.post("<?php echo U('Admin/Merchant/delGCate');?>",{'id':id},function(data){
                alert(data.msg);
                if('0'==data.errorCode){
