...
<!--左侧导航栏-->
    <div class="main-content">
        ...
        <div id="commodity_details">
            <h1><img src="/axd/Public/back_img/ico.png"><?php if(empty($_GET['id'])): ?>添加<?php else: ?>修改<?php endif; ?>商品
                <a href="javascript:history.go(-1);"><span><img src="/axd/Public/back_img/return.png"></span></a></h1>
            <div id="commodity_list">
                <!--通用信息_开始-->
                <form action="/axd/Admin/Goods/addEditGoods" method="post" id="goods_form" enctype="multipart/form-data" >

                    <div id="commodity_main" class="clear">
                        <table>
                            <input type="hidden" value="<?php echo ($goods['goods_id']); ?>" name="id">
                            <tr>
                                <th>所属分类：</th>
                                <td>
                                    <select name="sex" id="edit_sex">
                                        <option value="1" <?php if($goods['sex'] == 1): ?>selected="true"<?php endif; ?> >男神</option>
                                        <option value="2" <?php if($goods['sex'] == 2): ?>selected="true"<?php endif; ?> >女神</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <select id="edit_second">
                                        <?php if(is_array($second_list)): foreach($second_list as $key=>$s): ?><option value="<?php echo ($s["g_cate_id"]); ?>" <?php if($s['g_cate_id'] == $second_cate['g_cate_id']): ?>selected="true"<?php endif; ?> ><?php echo ($s["name"]); ?></option><?php endforeach; endif; ?>
                                    </select>
                                    <select id="edit_third" name="g_cate_id">
                                        <?php if(is_array($third_list)): foreach($third_list as $key=>$t): ?><option value="<?php echo ($t["g_cate_id"]); ?>" <?php if($t['g_cate_id'] == $goods['g_cate_id']): ?>selected="true"<?php endif; ?> ><?php echo ($t["name"]); ?></option><?php endforeach; endif; ?>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <th>商品名称</th>
                                <td><input type="text" name="name" value="<?php echo ($goods["name"]); ?>"></td>
                            </tr>
                            <tr>
                                <th>商品图片：</th>
                                <td>
                                    <?php if(empty($goods)): ?><img src="" id="to_img">
                                        <?php else: ?>
                                        <img src="/axd<?php echo ($goods["image"]); ?>" id="to_img"><?php endif; ?>
                            ...
                            <tr>
                                <th>淘宝链接：</th>
                                <td><input type="text" name="tb_link" value="<?php echo ($goods["tb_link"]); ?>"></td>
                            </tr>
                            <tr>
                                <th>有效日期：</th>
                                <td>
                                    <?php if(!empty($goods['end_time'])): ?><input type="text" readonly="true" id="dateinfo" name="end_time"
                                               value="<?php echo (date('Y-m-d',$goods["end_time"])); ?>">
                                        <?php else: ?>
                                        <input type="text" readonly="true" id="dateinfo" name="end_time"><?php endif; ?>
                                </td>
                            </tr>
                            <tr>
                                <th>文字介绍</th>
                                <td>
                                    <textarea name="text_content"><?php echo ($goods["text_content"]); ?></textarea>
                                </td>
                            </tr>
                            <tr>
                                <th>收藏数：</th>
                                <td><input type="text" value="<?php echo ($goods["collect_count"]); ?>" name="collect_count"></td>
                            </tr>
                            <tr>
                                <th>评论数：</th>
                                <td><input type="text" value="<?php echo ($goods["comment_count"]); ?>" name="comment_count"></td>
                            </tr>
                        </table>
                    </div>
...
<script>
    ...
    $(".spgl .splb").addClass("active");
    $('#li1_btn').click(function(){
        $('#goods_form').ajaxSubmit({
            url:"/axd/Admin/Merchant/addEditGoods",
            dataType: "json",
            beforeSend: function() {
