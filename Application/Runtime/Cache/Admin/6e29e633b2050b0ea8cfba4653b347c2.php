        ...
        <div id="myTabContent" class="tab-content">
            <div class="tab-pane fade in active" id="search">
                <div class="col-sm-4 col-xs-6">
                    <form class="bs-example bs-example-form" action="/axd/Admin/Index/expressList" id="sf">
                        <div class="input-group" style="margin-bottom: 3%;">
                            <input type="text" class="form-control" placeholder="快递名称搜索" value="<?php echo ($keyword); ?>" name="keyword">
                            <a onClick="document.getElementById('sf').submit();" class="input-group-addon"><i class="icon-search"></i></a>
                        </div>
                    </form>
                </div>
                <a href="<?php echo U('Admin/Index/addExpress');;?>" class='btn btn-sm btn-primary pull-left'>添加</a>
            </div>
            ...
            <table class="table table-bordered" id="list-table">
                ...
                <?php if(is_array($list)): foreach($list as $k=>$l): ?><tr>
                        ...
                        <td class="operation">
                            <!--<a href="<?php echo U('Admin/Index/addEditPartner',array('id'=>$l['partner_id']));?>">编辑</a>-->
                            <a href="javaScript:void(0);" onclick="delExpress('<?php echo ($l["id"]); ?>')">删除</a>
                        </td>
...
<script>
    ...
    $(".gggl .kdlb").addClass("active");
</script>
<script>
    //删除快递
    function delExpress(id){
        if(confirm("是否删除该数据")){
            $.post("<?php echo U('Admin/Index/delExpress');?>",{'id':id},function(data){
                alert(data.msg);
                if('0'==data.errorCode){
