...
<!--左侧导航栏-->
    <div class="main-content">
        ...
        <div id="commodity_details">
            <h1><img src="/axd/Public/back_img/ico.png"><?php if(empty($_GET['id'])): ?>添加<?php else: ?>修改<?php endif; ?>校趣帖子
                <a href="javascript:history.go(-1);"><span><img src="/axd/Public/back_img/return.png"></span></a></h1>
            <div id="commodity_list">
                <!--通用信息_开始-->
                <form action="/axd/Admin/Interact/addEditInteract" method="post" id="edit_form" enctype="multipart/form-data" >

                    <div id="commodity_main" class="clear">
                        <table>
                            <input type="hidden" value="<?php echo ($interact['interact_id']); ?>" name="id">
                            <tr>
                                <th>所属分类：</th>
                                <td>
                                    <select name="s_cate_id">
                                        <?php if(is_array($cate_list)): foreach($cate_list as $key=>$c): ?><option value="<?php echo ($c["s_cate_id"]); ?>"
                                            <?php if($interact['s_cate_id'] == $c['s_cate_id']): ?>selected="selected"<?php endif; ?>
                                ...
                                <td>
                                    <select name="school_id">
                                        ...
                                        <?php if(is_array($school_list)): foreach($school_list as $key=>$s): ?><option value="<?php echo ($s["school_id"]); ?>"
                                            <?php if($interact['school_id'] == $s['school_id']): ?>selected="selected"<?php endif; ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="is_video">
                                        <option value="0"
                                        <?php if($interact['is_video'] == 0): ?>selected="selected"<?php endif; ?>
                                        >否</option>
                                        <option value="1"
                                        <?php if($interact['is_video'] == 1): ?>selected="selected"<?php endif; ?>
                                        >是</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <th>标题：</th>
                                <td><input type="text" name="title" value="<?php echo ($interact["title"]); ?>"></td>
                            </tr>
                            <tr>
                                <th>作者</th>
                                <td><input type="text" name="user_nickname" value="<?php echo ($interact["user_nickname"]); ?>"></td>
                            </tr>
                            <tr>
                                <th>图片：</th>
                                <td>
                                    <?php if(empty($interact)): ?><img src="" id="to_img">
                                        <?php else: ?>
                                        <img src="/axd<?php echo ($interact["image"]); ?>" id="to_img"><?php endif; ?>
                            ...
                            <tr>
                                <th>日期：</th>
                                <td>
                                    <?php if(!empty($interact)): ?><input type="text" readonly="true" id="dateinfo" name="time"
                                               value="<?php echo (date('Y-m-d',$interact["time"])); ?>">
                                        <?php else: ?>
                                        <input type="text" readonly="true" id="dateinfo" name="time"><?php endif; ?>
                                </td>
                            </tr>
                            <tr>
                                <th>点赞数：</th>
                                <td><input type="text" value="<?php echo (intval($interact["praise_count"])); ?>" name="praise_count"></td>
                            </tr>
                            <tr>
                                <th>评论数：</th>
                                <td><input type="text" value="<?php echo (intval($interact["comment_count"])); ?>" name="comment_count"></td>
                            </tr>
                            <tr>
                                <th>分享数：</th>
                                <td><input type="text" value="<?php echo (intval($interact["share_count"])); ?>" name="share_count"></td>
                            </tr>
                            <tr>
                                <th>内容</th>
                                <td>
                                    <textarea name="content" id="container"><?php echo ($interact["content"]); ?></textarea>
                                </td>
                            </tr>
                        </table>
                    </div>
...
<!-- 配置文件 -->
<script type="text/javascript" src="/axd/Public/ueditor/ueditor.config.js"></script>
<!-- 编辑器源码文件 -->
<script type="text/javascript" src="/axd/Public/ueditor/ueditor.all.js"></script>
<!--编辑器语言加载-->
<script type="text/javascript" src="/axd/Public/ueditor/lang/zh-cn/zh-cn.js"></script>

<link type="text/css" rel="stylesheet" href="/axd/Public/ueditor/third-party/video-js/video-js.css"/>
<script language="javascript" type="text/javascript" src="/axd/Public/ueditor/third-party/video-js/video.js"></script>
    ...
    var ue = UE.getEditor('container',{
        ...
        toolbars: [
            ['fullscreen', 'source', '|', 'undo', 'redo','|', 'bold', 'italic', 'underline', 'fontborder',
                ...
                'justifyjustify',  '|', 'link', 'unlink', '|',
                'simpleupload', 'insertvideo' , 'map', '|',
                'horizontal', 'date', 'time','spechars',  '|', 'print', 'preview', 'searchreplace']
        ]
    });
</script>
<script>
    ...
    $('#li1_btn').click(function(){
        ...
        $('#edit_form').ajaxSubmit({
            url:"/axd/Admin/Interact/addEditInteract",
            dataType: "json",
            beforeSend: function() {
